#ifndef GUI_SHELL_SYSTEM_CONTROLLER_H_
#define GUI_SHELL_SYSTEM_CONTROLLER_H_
...
public:
    ...
    bool startNewsFeeds();
    bool startSudoku();
    bool startResSharing();
    bool updateWaveform(const QString & path);
    bool clearWaveform();
    bool startLedWatcher();
    ...
    void startPictureViewer(const QString &path);
    ...
    bool isPDFOrEPUB(const QString &path);

    bool startWritePad(const QString &path);
    bool hasTouch();

    bool startOnyxCloudClient();
    ...
    Service & onyxPdfReaderService();
    Service & coolReaderService();
    Service & htmlReaderService();

    Service & getEpubService();
    Service & getPdfService();
    Service & getDocService();
    Service & getFb2Service();
    Service & getChmService();
    void setFb2Service(Service & service);
    void setChmService(Service & service);
    void setDocService(Service & service);
    void setEpubService(Service & service);
    void setPdfService(Service & service);

Q_SIGNALS:
    /// Viewer state changed signal. This signal is emitted when
    /// a viewer is launched, launchs failed or succeed.
    void viewerStateChanged(shell::controller::ServiceState new_state);

    /// Content metadata ready.
    void metadataReady(const QString & path, bool result);
...
private:
    ...
    Service & news_feeds_Service() {return news_feeds_service_;}
